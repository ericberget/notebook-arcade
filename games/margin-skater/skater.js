...
function select(index,play=false){s=P.create(P.makeCruise(index));prev={x:s.x,y:s.y,angle:s.angle};cam=s.x-W*.27;acc=0;particles=[];trails=[];paused=false;clearInput();$('finish').hidden=true;$('pause-panel').hidden=true;$('intro').hidden=play;s.status=play?'playing':'ready';$('pause').disabled=!play;$('pause').textContent='Ⅱ pause';$('pause').setAttribute('aria-label','Pause game');$('page-name').textContent=`ZEN CRUISE · ${s.level.name.toUpperCase()}`;$('course-note').textContent=s.level.subtitle;$('announcement').classList.remove('show');toastLife=0;document.querySelectorAll('[data-level]').forEach(b=>b.setAttribute('aria-pressed',String(+b.dataset.level===index)));updateHUD();if(play){audio.unlock();canvas.focus({preventScroll:true});}}
function start(){$('intro').hidden=true;s.status='playing';$('pause').disabled=false;audio.unlock();canvas.focus({preventScroll:true});toast('just roll. the afternoon is yours.',2.8);}
$('start').onclick=start;
...
function retry(){if(s.level.zen){select(s.level.index,true);return;}if(s.status==='ready')return;if(s.status==='finished'){select(s.level.index,true);return;}clearInput();paused=false;$('pause-panel').hidden=true;$('pause').textContent='Ⅱ pause';$('pause').setAttribute('aria-label','Pause game');s.status='bail';s.bailTimer=0;s.combo=0;s.chain=0;s.airPoints=0;s.bails++;}
$('retry').onclick=retry;
...
canvas.addEventListener('pointerdown',e=>{if(e.button!==0||paused)return;e.preventDefault();if(s.status==='ready')start();canvas.setPointerCapture(e.pointerId);input.jump=true;s.held=true;});
for(const name of ['pointerup','pointercancel','lostpointercapture'])canvas.addEventListener(name,()=>input.jump=false);
...
function events(){for(const e of s.events){audio.play(e.type);if(e.type==='ollie'){burst(s.x,s.y,8,GRAY,.5);}if(e.type==='land'){burst(s.x,s.y,12,GRAY,.7);shake=reduced||s.level.zen?0:Math.min(3,e.impact/260);toast(s.level.zen?(e.label.includes('FLIP')?'a little flourish.':e.clean?'easy landing.':'keep rolling.'):`${e.label}${e.clean?' · CLEAN!':''}`,1);}
...
function updateHUD(){
  $('score').textContent=`${Math.floor((s.x-120)/10)} m`;
  $('best').textContent=`${Math.floor(s.time/60)}:${String(Math.floor(s.time)%60).padStart(2,'0')}`;
  $('stars').textContent=s.starsTotal;
  const progress=((s.x-120)%6000)/6000*100;
...
function skater(x,y,angle){
  ...
  path([[hip.x-3,hip.y],[neck.x-4,neck.y],[neck.x+6,neck.y+1],[hip.x+7,hip.y+1]],INK,2,true,'#78aca8');
...
function draw(alpha,dt){
  ...
  if(scenery.complete&&scenery.naturalWidth){
    ...
    for(let tile=first;tile*width-travel<W;tile++){
      ...
      ctx.translate(x+(Math.abs(tile)%2?width:0),offsetY-20);
      if(Math.abs(tile)%2)ctx.scale(-1,1);
      ctx.drawImage(scenery,0,0,width,h);ctx.restore();
    }
    ctx.globalAlpha=1;
  ...
  if(s.status==='playing'){text('nowhere to be. just here.',W-24,H-20,17,GRAY,.02,'right');}
}
