...
function cruiseHeight(x,index){return 438+Math.sin(x/680+index)*64+Math.sin(x/1370)*32;}
...
function bail(s,why){if(s.status!=='playing')return;if(s.level.zen){const ground=surface(s.x,s.level.ground);if(ground){Object.assign(s,{y:ground.y,angle:ground.angle,grounded:true,rail:null,vy:0,omega:0,speed:Math.max(120,s.vx*.85),airTime:0});bank(s);emit(s,'recover');return;}}s.status='bail';s.bailTimer=.85;s.bails++;s.combo=0;s.chain=0;s.airPoints=0;s.held=false;s.charge=0;s.buffer=0;emit(s,'bail',{why});}
...
function ollie(s,power){const a=s.rail?Math.atan2(s.rail.y2-s.rail.y1,s.rail.x2-s.rail.x1):s.angle;const impulse=s.level.zen?580+power*110:400+power*250;s.vx=s.level.zen?s.speed*Math.cos(a):Math.max(330,s.speed*Math.cos(a)+Math.sin(a)*impulse*.15);s.vy=s.speed*Math.sin(a)-(s.level.zen?impulse:Math.cos(a)*impulse);s.y-=2;s.grounded=false;s.rail=null;s.coyote=0;s.buffer=0;s.airTime=0;s.rotation=0;s.airPoints=40;s.airLabel=power>.65?'POWER OLLIE':'OLLIE';s.omega=-.35;s.compression=-.18;emit(s,'ollie',{power});}
...
function step(s,input={},dt=DT){
  ...
  s.pushCooldown=Math.max(0,(s.pushCooldown||0)-dt);
  if(s.pushStroke>0)s.pushStroke=Math.max(0,s.pushStroke-dt);
  ...
  if(s.grounded){
    ...
    if(!at){s.grounded=false;s.rail=null;s.airTime=0;s.rotation=0;}
      ...
      if(s.level.zen){
        // Gravity trades height for speed. Rolling resistance and air drag dissipate
        // energy gradually; neither pulls the rider back to a cruising set point.
        if(!s.rail&&s.speed<220&&s.pushCooldown===0&&s.charge<.05&&s.stumble===0){s.pushStroke=.36;s.pushCooldown=1.15;}
        const foot=!s.rail&&s.pushStroke>0?360*Math.sin(Math.PI*(1-s.pushStroke/.36)):0;
        const hillAssist=!s.rail?Math.max(0,185-s.speed)*4:0;
        const drag=(s.rail?9:5)+.00009*s.speed*s.speed;
        s.speed=Math.max(35,s.speed+(G*Math.sin(a)*.78+foot+hillAssist-drag)*dt);
      }else s.speed=clamp(s.speed+(G*Math.sin(a)*.68+push-rolling)*dt,180,690);
    ...
    if(hit){
      ...
      s.x=hit.x;s.y=hit.y;s.speed=clamp(s.vx*Math.cos(hit.angle)+s.vy*Math.sin(hit.angle),s.level.zen?35:205,s.level.zen?1400:690);
      const clean=error<.23&&impact<900;s.speed*=clean||s.level.zen?1:0.88;if(s.level.zen)s.speed*=.99;
  ...
  for(const o of s.level.obstacles){
    ...
    if(s.x+19>o.x&&s.x-19<o.x+o.w&&s.y>o.y-o.h+6&&s.y<o.y+45){
      if(s.level.zen){o.hit=true;s.bumps++;s.speed*=.72;s.vx*=.72;s.stumble=.7;s.compression=.6;emit(s,'bump');}
