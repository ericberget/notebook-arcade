...
window.XOMatch={you:L.teams[you],opponent:L.teams[opp],season:!!season,get,
 ...
 finish(a,b){if(finished)return;finished=true;let message='Quick Play final';let ok=false;
   if(season){const latest=L.load();if(latest&&latest.id===season.id&&(latest.rosterRevision||0)===(season.rosterRevision||0)&&L.finish(latest,fixture.id,a,b,totals)){if(highlight)latest.highlight=highlight;ok=L.save(latest);message=ok?'Final recorded · season saved':'Unable to save this result. Keep this page open.';season=latest;}else message=(latest?.id===season.id&&(latest.rosterRevision||0)!==(season.rosterRevision||0))?'Your squad changed in another tab. Start this matchup again with the updated roster.':'This matchup was already recorded or the season changed.';}
   ...
   if(season&&ok&&season.lastPerformance?.matchId===fixture.id){
     const points=document.createElement('div');points.className='match-development';
     const title=document.createElement('strong');title.textContent='+'+season.lastPerformance.earned+' performance points';points.appendChild(title);
     const line=document.createElement('div');line.textContent=L.performance(season).points+' banked · 100 points = +1 attribute';points.appendChild(line);
     const spend=document.createElement('a');spend.href='index.html#upgrades';spend.textContent=L.performance(season).points>=100?'Choose upgrades →':'View squad development →';points.appendChild(spend);
