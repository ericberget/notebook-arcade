...
assert.equal(run('state'),'gameover','real simulated game finishes');const saved=run('XOLeague.load()');assert.equal(saved.week,1);assert(saved.schedule[0].every(g=>g.score));assert(Object.values(saved.stats).some(s=>s.pass>0));
// Balance changes can produce a shutout; a replay exists only when our team scored a touchdown.
const ownTouchdowns=run('draftedSeason.rosters[0].reduce((n,p)=>n+(MATCH.getStats()[p.id]?.td||0),0)');assert.equal(Boolean(saved.highlight),ownTouchdowns>0);
const first=JSON.stringify(saved);run('gameOver()');assert.equal(JSON.stringify(run('XOLeague.load()')),first,'finishing twice does not duplicate totals');
// The carrier hooks connect the actual game outcome to the real player card totals.
run("XOLeague.save(draftedSeason); newGame(); introT=9999; loadPreset('slants'); MATCH.caught('WR-L'); giveBall(byName('WR-L')); captureReplay(); playFrame=6; Body.setPosition(byName('WR-L').body,{x:100,y:yardY(100)}); endPlay('td',null,{x:100,y:yardY(100)})");assert.equal(run("MATCH.getStats()[draftedSeason.rosters[0][5].id].receive"),75);assert.equal(run("MATCH.getStats()[draftedSeason.rosters[0][0].id].passTd"),1);
run('gameOver()');const scoringSave=run('XOLeague.load()');assert(scoringSave.highlight?.frames.length>1,'actual touchdown is recorded and saved');assert(scoringSave.highlight.frames.length<=181);assert(scoringSave.highlight.frames.every(f=>f.players.length===16&&f.ball.length===3));assert.equal(scoringSave.highlight.player,run('draftedSeason.rosters[0][5].name'));if(process.env.FOOTBALL_REPLAY_FIXTURE)fs.writeFileSync(process.env.FOOTBALL_REPLAY_FIXTURE,JSON.stringify(scoringSave));
