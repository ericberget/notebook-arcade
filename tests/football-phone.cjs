...
assert.equal(run('coachMode'),false);assert.equal(run('autoPlayDesign'),false);assert.equal(run('coachPlayReady()'),false);
run('setAutoPlayDesign(true)');assert(run('coachPlayReady() && !coachMode'),'designing plays does not change Player Mode');
run('setupPlay()');assert(run('coachPlayReady()'),'auto design draws the next down');
run('setAutoPlayDesign(false)');assert.equal(run('coachPlayReady()'),false,'turning auto design off clears its suggestion');
run('setAutoPlayDesign(true); loadPreset("slants"); setAutoPlayDesign(false)');assert.equal(run('selectedPlay'),'slants','turning off preserves a manually selected play');
run('setupPlay()');assert.equal(run('coachPlayReady()'),false,'disabled auto design leaves the next down blank');
...
canvas.handlers.pointercancel(point(1,280+at.x+15,at.y-sy-65));assert.equal(run('drawing'),null);assert.equal(run('coachPlayReady()'),false,'cancelled touch never keeps a partial route');
