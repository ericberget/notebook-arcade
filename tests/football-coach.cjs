...
assert.equal(run('coachMode'),false,'Player Mode is the default');assert.equal(run('autoPlayDesign'),false);assert.equal(run('coachPlayReady()'),false,'no automatically drawn routes');
run('setMode(true); introT=9999; hike()');assert.equal(run('state'),'presnap');assert(nodes.get('hike').disabled,'empty call cannot snap');
...
run("startDrive('you'); setMode(false); introT=9999; hike()");assert.equal(run('state'),'live','Player Mode retains manual snap');
